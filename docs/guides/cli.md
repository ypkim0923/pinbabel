...
`pinbabel-runs`는 MySQL에 저장된 최신 20건을, `pinbabel-run --id`는 안전하게 선별된 Embabel 이벤트와 최종 보고서를 보여준다. 실행 기록은 애플리케이션을 재시작해도 유지된다. Prompt, 응답, Tool 입력·출력과 credential은 실행 이벤트에 저장하지 않는다.
